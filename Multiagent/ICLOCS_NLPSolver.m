...
[problem,guess]=MultiAgentQUAVProblemNewt;          
...
if (simtime~=0 && ~re_opt) || mod(simtime,tstep)~=0 % For time instances without re-optimization
    ...
    tfpu=[solution.tf-crt_time;0;speval(solution.Up,1,crt_time);speval(solution.Up,2,crt_time);speval(solution.Up,3,crt_time)] ; % Return the interpolated solution
else
    ...
    if simtime~=0 % For later instances
        ...
        if solution.tf > opt_step
            ...
            mmmk = [speval(solution.Up,1,opt_step),speval(solution.Up,2,opt_step),speval(solution.Up,3,opt_step)];
            guess.inputs=[mmmk;solution.U(idx_new,:)];
        else
            guess.states(1,:)=x0';
        end
    ...
    if simtime~=0
        ...
        crt_time=simtime-solution.t_ref; % Current time in the frame of previous solution
        tfpu=[solution.tf-crt_time;computation_time;speval(solution.Up,1,crt_time);speval(solution.Up,2,crt_time);speval(solution.Up,3,crt_time)] ; % Return the interpolated solution
    else
