function [ tfpu ] = SingleAgentSolver(tpx0)

% SingleAgentSolver - Implement optimal control problem with ICLOCS in
...
disp('Stage 2 done')
if (simtime~=0 && ~re_opt|| simtime==0 && roz) % For time instances without re-optimization
    crt_time=simtime-solution.t_ref; % Current time in the frame of previous solution
    fintime = solution.tf-crt_time;
    u0 = [speval(solution.Up,1,crt_time),speval(solution.Up,2,crt_time),speval(solution.Up,3,crt_time)];
