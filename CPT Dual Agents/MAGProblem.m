...
problem.states.x0=[2.1 0 0 0 2.2 0, 2.1 0 0 0 1 0, 3 0 0 0 2.2 0, 3 0 0 0 1 0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[2.1 0 0 0 2.2 0, 2.1 0 0 0 1 0, 3 0 0 0 2.2 0, 3 0 0 0 1 0];
problem.states.x0u=[2.1 0 0 0 2.2 0, 2.1 0 0 0 1 0, 3 0 0 0 2.2 0, 3 0 0 0 1 0];

% State bounds. xl=< x <=xu
problem.states.xl=[2.1 -20 0 0 2.2 -50,  2.1 -20 0 0 1 -50, 3 -20 0 0 2.2 -50,  3 -20 0 0 1 -50];
problem.states.xu=[5.1 +20 0 0 10 +50,  5.1 +20 0 0 10 +50, 6 +20 0 0 10 +50,  6 +20 0 0 10 +50];
...
problem.states.xfl=[5.1 0 0 0 2.2 0, 5.1 0 0 0 1 0, 6 0 0 0 2.2 0, 6 0 0 0 1 0];
problem.states.xfu=[5.1 0 0 0 2.2 0, 5.1 0 0 0 1 0, 6 0 0 0 2.2 0, 6 0 0 0 1 0];
...
cc = (m_ag+m_pl/2)*gc;
problem.inputs.u0l=[-50 -50 -50  m_pl*gc/2, -50 -50 -50   m_pl*gc/2];
problem.inputs.u0u=[ 50 50 50  m_pl*gc/2,   50 50 50 m_pl*gc/2];

% Input bounds
problem.inputs.ul=[-50 -50 -50  m_pl*gc/2-5, -50 -50 -50   m_pl*gc/2-5];
problem.inputs.uu=[ 50 50 50  m_pl*gc/2+5,   50 50 50 m_pl*gc/2+5];


% Input constraint error bounds
problem.inputs.uConstraintTol= 0.1*ones(1,8);

% Guess the input sequences with [u0 uf]
guess.inputs(:,1)=[1 -1]; %u_x1
guess.inputs(:,2)=[0 0];
guess.inputs(:,3)=[cc cc];
guess.inputs(:,4)=[m_pl*gc/2 m_pl*gc/2]; %F_1
guess.inputs(:,5)=[1 -1]; %u_x2
guess.inputs(:,6)=[0 0];
guess.inputs(:,7)=[cc cc];
guess.inputs(:,8)=[m_pl*gc/2 m_pl*gc/2]; %F_2
...
problem.constraints.gl=[asep, 0, 0 0, 0.5 0.5];
problem.constraints.gu=[2*cl, 0, 0 0, inf inf];
problem.constraints.gTol=1e-2*ones(1,6);
...
stageCost = u_x1.*u_x1+u_y1.*u_y1+u_z1.*u_z1 +u_x2.*u_x2+u_y2.*u_y2+u_z2.*u_z2;
...
c=[abs(x1-x2), c1-0.9, sep1-1.2, sep2-1.2, o1,o2];
%------------- END OF CODE --------------
