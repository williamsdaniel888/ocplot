% main_OLStats - Script to collect statistics on open-loop simulation
% performance
%
...
for i = 1:length(N)
    ...
    [problem,guess]= MAXProblem;%SingleAgentQUAVProblem;%    % Fetch the problem definition
    % 
    tic
...
%     tic
%     [infoNLP,data,options]=transcribeOCP(problem,guess,options); % Format for NLP solver
%     [solution_HSCULIN,status,data] = solveNLP(infoNLP,data);      % Solve the NLP
%     [solution_HSCULIN]=output(problem,solution_HSCULIN,options,data,0);         % Output solutions
%     comp_time(4,i)=toc;
%     max_error(4,i) = max(max(solution_HSCULIN.Error));
%     max_rel_error(4,i) = max(max(solution_HSCULIN.ErrorRelative));
%     max_cst_error(4,i) = max(max(solution_HSCULIN.ConstraintError));
% 
%     sol=[];                             % Initialize solution structure
end
