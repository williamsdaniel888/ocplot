function [ tfpu ] = MAXSolver(tpx0)

% MAXSolver - Implement optimal control problem with ICLOCS in
...
%--------------------------------------------------------
global infoNLP data solution N opt_t_min opt_step max_error max_rel_error max_cst_error tstep
persistent solution_new loopcount fintime u0 roz
if(isempty(loopcount))
    loopcount = 0;
    [problem,guess]=MAXProblem;    
...
if (simtime~=0 && ~re_opt || simtime==0&&roz) %|| mod(simtime,tstep)~=0 % For time instances without re-optimization
    ...
    u0 = [speval(solution.Up,1,crt_time),speval(solution.Up,2,crt_time),speval(solution.Up,3,crt_time),speval(solution.Up,4,crt_time)];
...
%             options.nodes=length(options.tau)+1;  
%         end
        T_new_2=linspace(0,guess.tf,15);
        T_new_2(end)=guess.tf;
        options.tau=diff(T_new_2./(guess.tf))';
        options.nodes=length(options.tau)+1;
        disp('Stage 5 done')
    ...
    maxAbsError=1e9;maxConstraintError=1e9;
    i=1; imax=1;
    computation_time=0;
    while (any(maxAbsError>problem.states.xErrorTol) || any(maxConstraintError>problem.constraintErrorTol)) && i<=imax && solution.status.status~=2
        ...
        if(max(max(abs(solution_new.Error)))>max_error)
            max_error = max(max(abs(solution_new.Error)));
        end
        if(max(max(abs(solution_new.ErrorRelative)))>max_rel_error)
            max_rel_error = max(max(abs(solution_new.ErrorRelative)));
        end
        if(max(max(solution_new.ConstraintError))>max_cst_error)
            max_cst_error = max(max(solution_new.ConstraintError));
        end
    ...
    if solution.status.status~=0&&solution.status.status~=1&&solution.status.status~=-1
        warning('Ipopt unable to find optimal solution');
    elseif solution.status.status==-1
        warning('Ipopt exceeded max number of iterations');
    end
