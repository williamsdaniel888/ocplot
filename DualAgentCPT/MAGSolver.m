...
if (simtime~=0 && ~re_opt || simtime==0&&roz) %|| mod(simtime,tstep)~=0 % For time instances without re-optimization
    ...
    u0 = [speval(solution.Up,1,crt_time),speval(solution.Up,2,crt_time),speval(solution.Up,3,crt_time),speval(solution.Up,4,crt_time),speval(solution.Up,5,crt_time),speval(solution.Up,6,crt_time),speval(solution.Up,7,crt_time),speval(solution.Up,8,crt_time)];
...
%         [ options, guess ] = doMeshRefinement( options, problem, guess, data, solution_new, i );
    ...
    elseif solution.status.status==-1
        warning('Ipopt max number of iterations exceeded')
    end
